...
impl MessageBusImpl {
    ...
    pub async fn register(
        ...
        match mode {
            HandlerMode::Broadcast => {
                self.broadcast
                    .entry(topic.main.clone())
                    .or_default()
                    .push(handler.clone());
                debug!(
                    "Registered Broadcast handler: {} / {}",
                    topic.main, topic.sub
                );
            }
            HandlerMode::Request => {
                self.request
                    .insert((topic.main.clone(), topic.sub.clone()), handler.clone());
                ...
                .map(|h| {
                    ...
                    async move {
                        if let Err(e) = h.handle(msg.clone()).await {
                            error!(
                                "Broadcast handler error on topic {}: {:?}",
                                msg.topic.main, e
                            );
                        } else {
    ...
    pub async fn request(&self, msg: BusMessage, dur: Option<Duration>) -> Result<BusMessage> {
        let fut = async {
            if let Some(h) = self
                .request
                .get(&(msg.topic.main.clone(), msg.topic.sub.clone()))
            {
                match h.handle(msg.clone()).await {
                    Ok(resp) => {
                        debug!(
                            "Request handler success for topic {} / {}",
                            msg.topic.main, msg.topic.sub
                        );
                        Ok(resp)
                    }
                    Err(e) => {
                        error!(
                            "Request handler error on topic {} / {}: {:?}",
                            msg.topic.main, msg.topic.sub, e
                        );
                        Err(e)
                    }
                }
                ...
                .request
                .get(&(msg.topic.main.clone(), "default".into()))
            {
                match h.handle(msg.clone()).await {
                    Ok(resp) => {
                        debug!(
                            "Request handler success for topic {} / default",
                            msg.topic.main
                        );
                        Ok(resp)
                    }
                    Err(e) => {
                        error!(
                            "Request handler error on topic {} / default: {:?}",
                            msg.topic.main, e
                        );
                        Err(e)
                    }
                ...
                warn!(
                    "No Request handler for topic={} sub_topic={}",
                    msg.topic.main, msg.topic.sub
                );
        ...
        match timeout(timeout_duration, fut).await {
            ...
            Err(_) => {
                error!(
                    "Request timeout for topic {} / {} after {:?}",
                    msg.topic.main, msg.topic.sub, timeout_duration
                );
