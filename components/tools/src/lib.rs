...
pub fn replace_all_column_names(sql: &str) -> Result<String, String> {
    ...
    let mut statements = Parser::parse_sql(&dialect, &preprocessed_sql).map_err(|e| e.to_string())?;
    ...
    // 将替换后的 SQL 文本转换回字符串
    let replaced_sql = statements.iter().map(|stmt| stmt.to_string()).collect::<Vec<_>>().join("; ").replace("...", "?");
