...
impl UserService {
    ...
    pub async fn update_user(
        &self,
        user_id: i64,
        updates: crate::application::auth::dtos::UpdateUserRequest,
    ) -> DomainResult<()> {
        let mut user = self
            .user_repository
            .find_by_id(user_id)
        ...
        if let Some(ref email) = updates.email {
            if let Some(existing_user) = self.find_by_email(email).await? {
                if existing_user.id != Some(user_id) {
                    return Err(DomainError::ValidationError {
        ...
        if updates.nick_name.is_some() || updates.avatar.is_some() {
            user.update_profile(
                updates.nick_name.or(user.nick_name.clone()),
                updates.avatar.or(user.avatar.clone()),
            );
        }
