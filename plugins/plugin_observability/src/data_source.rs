...
impl DataProcessor {
    ...
    pub fn prometheus_to_dataframe(data: &serde_json::Value) -> PolarsResult<DataFrame> {
        ...
        if let Some(result) = data.as_array() {
            for metric in result {
                ...
                let label_str = metric.get("metric")
    ...
    /// 简单异常检测（基于阈值）
    pub fn detect_anomalies_simple(_df: &DataFrame, _value_col: &str, _threshold: f64) -> Result<Vec<bool>, String> {
...
impl HttpDataSource {
    ...
    pub async fn get_request(
        ...
        let body = response.text().await.map_err(|e| format!("Failed to parse response: {}", e))?;
...
pub fn create_data_source(config: DataSourceConfig) -> Box<dyn DataSource> {
    match config {
        DataSourceConfig::Prometheus { base_url, cookie } => {
            ...
            headers.insert("Cookie", HeaderValue::from_str(&format!("grafana_session={}", cookie)).unwrap());
...
impl DataSource for PrometheusDataSource {
    async fn test_connection(&self) -> Result<ConnectionTestResult, String> {
        self.http_source.health_check("api/datasources/proxy/1/api/v1/status/config").await
    }
    ...
    async fn query(&self, query: &str, params: Option<HashMap<&str, String>>) -> Result<QueryResult, String> {
        ...
        let result = self.http_source.get_request(
...
impl DataSource for ClinicDataSource {
    async fn test_connection(&self) -> Result<ConnectionTestResult, String> {
        self.http_source.health_check("clinic/api/v1/dashboard/clusters2?limit=1").await
    }

    async fn query(&self, query: &str, _params: Option<HashMap<&str, String>>) -> Result<QueryResult, String> {
