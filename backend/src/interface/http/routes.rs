...
use crate::application::ddl_precheck::{DDLPrecheckApplicationService, DDLPrecheckApplicationServiceImpl};
...
use crate::interface::http::slowlog_controllers::{SlowlogController, SlowlogAppState};
...
use crate::interface::http::settings_controllers::{SettingsController, SettingsAppState};
use crate::interface::http::notifications_controllers::{NotificationsController, NotificationsAppState};
use crate::interface::http::table_controllers::{TableController, TableAppState};
use crate::interface::http::database_controllers::{DatabaseController, DatabaseControllerState};
use crate::interface::http::sql_editor_controllers::{SqlEditorController, SqlEditorControllerState};

/// 创建所有 API 路由
/// 使用应用服务进行业务逻辑处理
pub fn create_api_routes() -> Router {
    // 创建应用服务实例
    let slowlog_service: Arc<dyn SlowlogApplicationService> = Arc::new(SlowlogApplicationServiceImpl::new());
    let ddl_service: Arc<dyn DDLPrecheckApplicationService> = Arc::new(DDLPrecheckApplicationServiceImpl::new());
    let settings_service: Arc<dyn SettingsApplicationService> = Arc::new(SettingsApplicationServiceImpl::new());
    ...
    let database_service: Arc<dyn DatabaseApplicationService> = Arc::new(DatabaseApplicationServiceImpl::new());
    let sql_editor_service: Arc<dyn EditorApplicationService> = Arc::new(SqlEditorApplicationServiceImpl::new());
