...
use crate::application::services::{
    CreateQueryRequest, EditorApplicationService, ExecuteQueryRequest, UpdateQueryRequest,
};
...
impl EditorController {
    ...
    async fn list_queries(
        ...
        match state
            .editor_service
            .list_queries(&database_id, pagination)
            .await
        {
    ...
    fn handle_error(
        error: crate::domain::shared::DomainError,
    ) -> (StatusCode, Json<ErrorResponse>) {
        use crate::domain::shared::DomainError;

        let (status, message) = match error {
            DomainError::NotFound { resource } => {
                (StatusCode::NOT_FOUND, format!("资源未找到: {}", resource))
            }
            ...
            DomainError::ExternalServiceError { service } => (
                StatusCode::BAD_GATEWAY,
                format!("外部服务错误: {}", service),
            ),
