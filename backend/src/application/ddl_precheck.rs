...
use crate::domain::{
    shared::DomainResult,
    ddl_precheck::{
        DDLAnalysisResult, LossyStatus,
        DDLPrecheckDomainService
    }
};
...
impl DDLPrecheckApplicationServiceImpl {
    ...
    /// 解析外部插件响应，提取问题列表
    fn extract_issues_from_plugin_response(plugin_response: &plugin_lossy_ddl::AnalysisResult) -> Vec<String> {
    ...
    async fn precheck_ddl(&self, request: DDLPrecheckRequest) -> DomainResult<DDLPrecheckResponse> {
        ...
        if !ddl_statement.is_ddl() {
            ...
            return Ok(DDLPrecheckResponse {
                ...
                recommendations: vec!["Please provide a valid DDL statement (CREATE, ALTER, DROP, etc.)".to_string()],
                error: None,
        ...
        // 5. 创建分析结果聚合根
        let analysis_result = DDLAnalysisResult::new(
            ddl_statement,
            lossy_status,
            risk_level,
            issues,
...
impl DDLPrecheckApplicationServiceImpl {
    /// 调用外部分析插件
    async fn call_external_analysis(&self, request: &DDLPrecheckRequest) -> DomainResult<plugin_lossy_ddl::AnalysisResult> {
        use microkernel::platform::message_bus::{BusMessage, MessageBus, GLOBAL_MESSAGE_BUS};
        
        // 构造 bus 消息
        let bus_msg = BusMessage::ok("ddl_precheck", serde_json::json!({
            "sql": request.sql,
            "collation_enabled": request.collation_enabled
        }));
        
        // 通过 bus 分发并等待响应
        let reply = GLOBAL_MESSAGE_BUS.request(bus_msg).await.map_err(|e| {
        ...
        // 解析响应
        let plugin_result: plugin_lossy_ddl::AnalysisResult = reply.unwrap_data().map_err(|e| {
